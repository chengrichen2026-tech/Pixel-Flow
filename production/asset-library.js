(() => {
  ...
  function mediaList(library, kind, query) {
    ...
    return `<section class="pf-media-grid pf-media-grid--${kind}">${items.map((item) => `<article class="pf-media-item" data-id="${item.id}"><div data-thumb="${item.assetId}">载入中</div><footer class="pf-management-card-meta"><strong title="${escapeHtml(item.name)}">${escapeHtml(item.name)}</strong><span><button data-action="media-edit" data-id="${item.id}" aria-label="编辑素材 ${escapeHtml(item.name)}" title="编辑">${libraryCardIcon("rename")}</button><button class="danger" data-action="media-delete" data-id="${item.id}" aria-label="删除素材 ${escapeHtml(item.name)}" title="删除">${libraryCardIcon("delete")}</button></span></footer>${promptTagBadges(item)}</article>`).join("") || "<p class=\"pf-empty\">还没有图片素材</p>"}</section>`;
  }

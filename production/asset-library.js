(() => {
  ...
  function formTemplate(root = panel) {
    ...
    return {
      ...
      count: Math.min(4, Math.max(1, Number(data.get("count") || 1))), generationMode: ["browser", "team"].includes(String(data.get("generationMode"))) ? String(data.get("generationMode")) : "api", teamImageModel: data.get("teamImageModel") === "sunburst" ? "sunburst" : "flare"
    };
  }
  ...
  function templateUsageList(library, query) {
    ...
    return `<section class="pf-usage-templates">${items.map((item) => `<article><strong>${escapeHtml(item.name)}</strong><p>${item.count} 张 · ${escapeHtml(item.aspectRatio)} · ${item.generationMode === "api" ? "API" : item.generationMode === "team" ? `团队生图 · ${item.teamImageModel === "sunburst" ? "Sunburst" : "Flare"}` : "浏览器"}</p><button data-action="template-use" data-id="${item.id}">创建任务</button></article>`).join("") || "<p class=\"pf-empty\">还没有已保存的生图模板</p>"}</section>`;
  }

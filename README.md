...
- 团队即时调度：妙搭提交任务后立即写入 Cloudflare Queue，消费者以最大并发3开始处理；每分钟 Cron 继续作为漏单兜底，不再固定等待下一分钟。
...
- 推荐方案：使用妙搭云端任务箱提供固定地址，由 Cloudflare Queue 即时唤醒云端 Worker；Mac 无需在线，Cron 每分钟负责漏单兜底。

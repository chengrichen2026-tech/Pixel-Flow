...
const tools=[
 ...
 {name:"pixel_flow_create_task",description:"Create a generation task with prompt, mode, team image model, ratio, position and optional source node ids.",inputSchema:{type:"object",properties:{prompt:{type:"string"},generationMode:{type:"string",enum:["api","browser","team"]},teamImageModel:{type:"string",enum:["flare","sunburst"]},aspectRatio:{type:"string"},x:{type:"number"},y:{type:"number"},sourceNodeIds:{type:"array",items:{type:"string"}},requestId:{type:"string"},expectedRevision:{type:"integer",minimum:0},...target},required:["requestId","expectedRevision"],additionalProperties:false}},

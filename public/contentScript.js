...
(() => {
  ...
  async function uploadImages(fileInput, images) {
    ...
    while (countComposerAttachments() < previousAttachmentCount + images.length || composerIsUploading()) {
      if (Date.now() - uploadStartedAt > 6e4) {
        throw new AdapterError("upload_failed", "\u53C2\u8003\u56FE\u6CA1\u6709\u51FA\u73B0\u5728 ChatGPT \u8F93\u5165\u533A");
      }
  ...
  async function submitTask(input) {
    ...
    try {
      ...
      if (form instanceof HTMLFormElement) form.requestSubmit(sendButton);
      try {
        await waitUntil(
          () => document.querySelectorAll(userSelector).length > previousUserTurnCount,
          5e3,
          new Error("requestSubmit was not accepted")
        );
      } catch {

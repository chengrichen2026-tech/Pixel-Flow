...
(() => {
  ...
  function submittedTurnIsStillVisible(previousUserTurnCount, prompt) {
    ...
    const submittedText = normalizeTurnText(prompt);
    if (!submittedText) return true;
    const latestText = normalizeTurnText(turns.at(-1)?.innerText || turns.at(-1)?.textContent || "");
    return latestText.includes(submittedText);
  }
  function createCompletionConversationCheck(expectedConversationUrl, previousUserTurnCount, prompt) {
    ...
    return () => {
      if (lockedConversationUrl) {
        ...
        if (!location.href.startsWith("https://chatgpt.com/") || !submittedTurnIsStillVisible(previousUserTurnCount, prompt)) {
          throw new AdapterError("conversation_unavailable", "\u7B49\u5F85\u671F\u95F4\u6807\u7B7E\u9875\u5207\u6362\u5230\u4E86\u5176\u4ED6 ChatGPT \u5BF9\u8BDD");
        }
  ...
  async function uploadImages(fileInput, images) {
    ...
    const previousAttachmentCount = countComposerAttachments();
    const transfer = new DataTransfer();
    for (const image of images) transfer.items.add(decodeImage(image));
    ...
    await waitUntil(
      () => countComposerAttachments() >= previousAttachmentCount + images.length && !composerIsUploading(),
      2e4,
      new AdapterError("upload_failed", "\u53C2\u8003\u56FE\u6CA1\u6709\u51FA\u73B0\u5728 ChatGPT \u8F93\u5165\u533A")
    );
  }
  ...
  if (contentScriptScope.__gptNodeCanvasAdapterVersion !== CHATGPT_ADAPTER_VERSION) {
    ...
    chrome.runtime.onMessage.addListener((raw, _sender, sendResponse) => {
      ...
      if (raw.type !== "EXECUTE_IN_CHATGPT_V3") return false;
      const message = raw;

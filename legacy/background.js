...
function resolveTaskConversationUrl(message, senderUrl) {
  ...
  return senderUrl?.startsWith("https://chatgpt.com/") ? senderUrl : message.conversationUrl;
}
...
async function applyTaskMessage(project, message, saveAsset) {
  ...
  if (message.type === "TASK_ERROR") {
    const manual = ["login_required", "verification_required", "usage_limited", "conversation_unavailable"].includes(message.reason);
    return updateTask(project, message.taskId, (task) => ({
      ...task,
...
var TabRegistry = class {
  ...
  async open(taskId, conversationUrl) {
    const mapped = await this.ensure(taskId, conversationUrl);
...
chrome.runtime.onMessage.addListener((raw, sender, sendResponse) => {
  ...
  if ((message.type === "TASK_RESULT" || message.type === "TASK_ERROR") && !message.persisted) {
...
void updateScheduler(async () => void 0);
